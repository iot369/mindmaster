...
for(var i=0;i<comments.length;i++){
    ...
    var ps=comments[i].parentNode.getAttribute('transform').match(/\((\S*|\S*\s\S*)\)/)[1].split(/ |,/);
    var x=parseFloat(cs[0])+parseFloat(ps[0]);
    var y=parseFloat(ps[1]);
    x=x.tosuitsvg();
    y=y.tosuitsvg();
...
//        window.getSelection ? window.getSelection().removeRange(window.getSelection().re):document.selection.empty();
        ...
        var ps=notes[i].parentNode.getAttribute('transform').match(/\((\S*|\S*\s\S*)\)/)[1].split(/ |,/);
        var x=parseFloat(cs[0])+parseFloat(ps[0]);
        var y=parseFloat(ps[1]);
        x=x.tosuitsvg();
        y=y.tosuitsvg();
        out.setAttribute('transform','translate('+x+','+y+')');
...
for(var i=0;i<links.length;i++){
    ...
    var ps=links[i].parentNode.getAttribute('transform').match(/\((\S*|\S*\s\S*)\)/)[1].split(/ |,/);
    var x=parseFloat(cs[0])+parseFloat(ps[0]);
    var y=parseFloat(ps[1]);
    x=x.tosuitsvg();
    y=y.tosuitsvg();
...
function init(id, obj) {
    ...
    if (childs.length !== 0) {
        ...
        if (document.querySelector("g[ed\\:togtopicid='" + id + "']")) {
            ...
            var tog = document.querySelector("g[ed\\:togtopicid='" + id + "']").children[0].getAttribute('xlink:href').slice(1);
...
for(var i=0;i<shape.length;i++){
    shape[i].onclick=function () {
        ...
        this.children[0].setAttribute('xlink:href','#'+type);
        obj.togtype=type;
        checkID=obj;
